    ...
    (cl-letf (((symbol-function 'executable-find)
              ...
              ((symbol-function 'hyprland-zen--usable-executable-p)
               (lambda (path)
                 (setq seen path)
                 (and (stringp path)
                      (string-match-p "browser/native-host/hyprland-zen-host$" path)))))
      (should (equal (hyprland-zen--resolve-host-command)
                     '("/repo/browser/native-host/hyprland-zen-host")))
      (should (string-match-p "browser/native-host/hyprland-zen-host$" seen)))))
